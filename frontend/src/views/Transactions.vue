  ...
  <div class="page-container">
    ...
    <div class="filter-bar aesthetic-card">
      <div class="ledger-tabs">
        <span v-for="tab in ledgerTabs" :key="tab.value" :class="['tab', { active: currentLedger === tab.value }]"
      ...
      <n-form :model="newTx" @submit.prevent="handleAddSubmit">
        ...
        <div style="display: flex; gap: 16px;">
          <n-form-item label="金额" path="amount" style="flex: 1">
            <n-input-number v-model:value="newTx.amount" placeholder="0.00" :min="0" :precision="2"
              style="width: 100%" />
          </n-form-item>
          <n-form-item label="类型" path="type" style="flex: 1">
            <n-select v-model:value="newTx.type"
        ...
        <div v-if="selectedFamilyId"
          style="padding: 12px; background: rgba(255, 107, 53, 0.1); border-radius: 8px; margin-top: 8px;">
          <span style="color: #ff6b35;">📦 当前记账到：</span>
          <span>{{families.find(f => f.id === selectedFamilyId)?.name || '家庭账本'}}</span>
        </div>
        <div v-else style="padding: 12px; background: rgba(0, 184, 212, 0.1); border-radius: 8px; margin-top: 8px;">
          <span style="color: #00b8d4;">👤 当前记账到：</span>
          <span>个人账本</span>
        </div>
...
const onFamilyChange = () => {
  newTx.value.familyId = selectedFamilyId.value
  // 保存到本地存储
...
const handleAddSubmit = async () => {
  ...
  try {
    ...
    // reset
    newTx.value = { description: '', amount: null, type: 'expense', category: '其他', ledger: '日常账本', familyId: null }
    fetchTransactions()
  } catch (error) {
    message.error('记账失败')
  } finally {
